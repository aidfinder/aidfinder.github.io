...
class Json_Controller{
	...
	public function __construct(){
		...
		{	
			...
			case 'requestArchitectService':
				  $this->requestArchitectService();
		    break;
		    case 'updateMyMinimum':
				  $this->updateMyMinimum();
		    break;
		    case 'addmake':
				  $this->addmake();
		    break;
		    ...
		    break;
		    case 'municipalitydropdown2';
		    	$this->municipalitydropdown2();
		    break;
		    ...
		    break;
		    case 'archi_signupbutton':
				  $this->archi_signupbutton();
		    break;
		    case 'submitAdRequest':
    ...
    public function addMyMinimum(){
		include 'ajax_model.php';
		$data = array();
		$error = false;

		if(empty($_POST['min_rate'])){
			$data['status'] = "No input found!";
			$error = true;
		}
		
		$query = $model->filterExistingMinimum( $_POST['myID'] );
		if( $query == true ){
			$data['status'] = "You have already submitted your minimum rate!";
			$error = true;
		}

		if(!$error){
			$query = $model->addMyMinimum($_POST);

			if($query){
				$data['status'] = 1;
				$data['redirect_page'] = '?page=viewMyService';
			}else{
	...
	public function updateMyMinimum(){
		include 'ajax_model.php';
		$data = array();
		$error = false;

		if(empty($_POST['myUpdateID'])){
			$data['status'] = "No input found!";
			$error = true;
		}
		
		if(!$error){
			$query = $model->updateMyMinimum($_POST);
    ...
    public function requestArchitectService(){
		include 'ajax_model.php';
		$data = array();
		$error = false;

		if( empty($_POST['architect'])
			||empty($_POST['subcat'])
			||empty($_POST['barangay'])
			||empty($_POST['txtDate'])
			||empty($_POST['follow_Date'])  ){
			$data['status'] = "( Empty Fields Found ! )";
			$error = true;
		}

		$query = $model->filterExistingRequest( $_POST['myID'],$_POST['architect']);
		if( $query == true ){
			$data['status'] = "( You have already submitted your request on this architect ! )";
			$error = true;
		}

		if(!$error){
			$query = $model->requestArchitectService($_POST);

			if($query){
				$data['status'] = 1;
				$data['redirect_page'] = '?page=architecture&accomplished';
			}else{
				$data['status'] = 'There was an error while inserting data';
			}
		}
	...
	function notificationdropdown()
	{
		...
		$error = false;
		$lst = array();
		$data['results'] = $model->notificationdropdown($_POST);
		if($data['results']){
			foreach($data['results'] as $row)
			{		
				$lst[] = array('id'=>$row['bid_id'],'ptitle'=>$row['project_title'],'contract'=>$row['contractor_username']);	
			}
		}
		echo json_encode($lst);
	}
	...
	function municipalitydropdown2()
	{
		include 'ajax_model.php';
		$error = false;
		$lst = array();
		$data['results'] = $model->municipalitydropdown2($_POST);
		if($data['results']){
			foreach($data['results'] as $row)
			{		
				$lst[] = array('id'=>$row['mun_id'],'name'=>$row['name']);	
	...
	public function submitAdRequest(){
		include 'ajax_model.php';
		$data = array();
		$error = false;

		if( empty($_POST['emailad'])){
			$data['status'] = "( Don't leave the field empty ! )";
			$error = true;
		}
		$query = $model->filterExistingEmailForAdReq( $_POST['emailad'] );
		if( $query == true ){
		...
		if(!$error){
			$query = $model->submitAdRequest($_POST);

			if($query){
				$data['status'] = 1;
				$data['redirect_page'] = '?page=homes';
			}else{
				$data['status'] = 'There was an error while inserting data';
			}
		}
	...
	public function submit_btn(){
		...
		$query = $model->filterExistingBid( $_POST['projectID'],$_POST['myID'] );
		if( $query == true ){
			$data['status'] = "You have already submitted your bid on this project !";
			$error = true;
		}

		if(!$error){
			$query = $model->submit_btn($_POST);

			if($query){
				$data['status'] = 1;
				$data['redirect_page'] = '?page=projectLeads';
			}else{
				$data['status'] = 'There was an error while inserting data';
			}
		}
	...
	public function archi_signupbutton(){
		include 'ajax_model.php';
		$data = array();
		$error = false;
		...
		if( empty($_POST['expertise_id']) ||
			...
			$data['status'] = "( Don't leave any field empty ! )";
			$error = true;
		}
		$query = $model->filterExistingArchitect( $_POST['fname'],$_POST['lname'] );
		if( $query == true ){
			$data['status'] = "Someone already obtained this name !";
			$error = true;
		}
		$query = $model->filterExistingArchitectEmail( $_POST['email']);
		if( $query == true ){
			$data['status'] = "Someone already obtained this email !";
			$error = true;
		}

		if(!$error){
			$query = $model->archi_signupbutton($_POST);

			if($query){
				$data['status'] = 1;
				$data['redirect_page'] = '?page=architect_signup';
			}else{
				$data['status'] = 'There was an error while inserting data';
			}
		}
