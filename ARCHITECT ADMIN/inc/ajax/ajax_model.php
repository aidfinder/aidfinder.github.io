...
class Json_Model{
    ...
    public function submit_btn($post){
        $query = $this->query('INSERT INTO tbl_bids VALUES(
                            null,
                            "'.$post['projectID'].'",
                            "'.$post['myID'].'",
                            CURDATE(),
    ...
    public function addMyMinimum($post){
        $query = $this->query('INSERT INTO tbl_myminimum_rate VALUES(
                            null,
                            "'.$post['min_rate'].'",
                            "'.$post['myID'].'",
                            1
    ...
    public function updateMyMinimum($post){
        $query = $this->query('UPDATE tbl_myminimum_rate SET
                           myminimum = "'.$post['myCurrentMinimum'].'"
                           WHERE min_id = "'.$post['myUpdateID'].'"
                             ');
        if($query){
            return 1;
        }else{
    ...
    public function archi_signupbutton($post){
    ...
    public function filterExistingArchitect($s_fname,$s_lname){
    ...
    public function filterExistingMinimum($myID){
        $query = $this->query('SELECT * FROM tbl_myminimum_rate WHERE builder_id = "'.$myID.'"');
        if ( $result = $this->fetch_data($query) ) {
            return true;
        }else{
            return false;
    ...
    public function filterExistingArchitectEmail($s_email){
        $query = $this->query('SELECT * FROM tbl_builder WHERE b_email = "'.$s_email.'" ');
        if ( $result = $this->fetch_data($query) ) {
            return true;
        }else{
            return false;
    ...
    public function filterExistingBid($projectID,$myID){
        $query = $this->query('SELECT * FROM tbl_bids WHERE project_id = "'.$projectID.'" and contractor_id= "'.$myID.'" ');
        if ( $result = $this->fetch_data($query) ) {
            return true;
        }else{
            return false;
    ...
    public function requestArchitectService($post){
        $query = $this->query('INSERT INTO tbl_request VALUES(
                            null,
                            "'.$post['myID'].'",
                            "'.$post['subcat'].'",
                            CURDATE(),CURTIME(),
    ...
    public function filterExistingRequest($myID,$architect){
        $query = $this->query('SELECT * FROM tbl_request WHERE user_id = "'.$myID.'" and builder_id = "'.$architect.'" and req_status=0 ');
        if ( $result = $this->fetch_data($query) ) {
            return true;
        }else{
            return false;
    ...
    public function municipalitydropdown2($post){
            $data="";

            $query = $this->query('SELECT * from tbl_municipality where prov_id= "'.$post['idr2'].'" ');
    ...
    public function notificationdropdown($post){
            $data="";

            $query = $this->query('SELECT * FROM tbl_clientproject c,tbl_bids b,tbl_contractor o where b.project_id=c.clientproject_id and b.contractor_id=o.contractor_id and c.user_id="'.$post['the_user_id'].'" and b.dummy_status_bid=0 and c.project_status=1 ');
           ...
           $conn = new mysqli("localhost","root","","panday");
            ...
            if($this->num_rows($query)>0){ 
                ...
                while($yu = $this->fetch_data($query)){
                        ...
                        $sql='UPDATE tbl_bids SET dummy_status_bid=1 WHERE dummy_status_bid=0 and project_id="'.$yu['clientproject_id'].'"'; 
