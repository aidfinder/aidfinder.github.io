...
function filtered_municipality2(){
	//
	 var ids2 = $('#provincess2').val();
		$.ajax({
	            type:'POST',
	            url: baseUrlAction() + '?btn=municipalitydropdown2',
	            context: this,
	            dataType: 'json',
	            data: { 
	                    idr2 : ids2
	            ...
	            success: function(data) { 
		                ...
		                if(jQuery.isEmptyObject(data)){
		                	 row +='<select disabled id="municipalityz2" name="municipalityz2" style="width:100%;height:45px;">';
		                     row +='<option value="">--No Filtered Result--</option>';
		                	 row +='</select>';
		                }else{
		                	row +='<select id="municipalityz2" name="municipalityz2" style="width:100%;height:45px;">';
		                    $.each(data, function(key,value){        
		                    row +='<option value="'+value.id+'">' + value.name+'</option>';
		                    });   
		                    row +='</select>';
		                } 
		                $('#div_mun').html(row).fadeIn('slow').slideDown('slow');     
	                }
	        });  
		 setTimeout(function(){ //filtered_barangay();
		 }, 1500)
		...
		$.ajax({
	            ...
	            success: function(data) { 
		                ...
		                if(jQuery.isEmptyObject(data)){
		                	...
		                	$.each(data, function(key,value){
							row +='<li><a href="./CONTRACTOR/ajax_upload/bid_documents/'+value.id+'.pdf" download>'+'<strong>'+value.ptitle+'</strong><br/><small><em>' + value.contract+'&nbsp;submitted a bid on your project</em></small></a>';
		                    });   
		                } 
...
$('#requestArchitectService').bind('click', function(eve){ 
		eve.preventDefault();   
		$('#requestArchitectService').show('slow').html('<i class="fa fa-spinner"></i>Requesting...');
				        $.ajax({ 
					            type:'POST',
					            url: baseUrlAction() + '?btn=requestArchitectService',
					            context: this,
					            dataType: 'json',
					            data: { 
					            ...
					            success: function(data) { 
					                if(parseFloat(data.status)==1){          
								          $('#requestArchitectService').html('Submit').fadeIn('slow');
								          $('.alert').removeClass('alert-danger');
								          $('.message').addClass('alert').show().slideDown('slow');
								          $('.alert').addClass('alert-success').show().slideDown('slow');
								          $('.message').html("Successfully Submitted Request!");
					                      setTimeout(function(){
					                            window.location.href = data.redirect_page ;
					                      }, 1500)
					                }else { 
